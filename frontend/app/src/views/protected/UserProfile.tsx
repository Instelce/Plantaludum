...
function UserProfile() {
  ...
  const updateUserMutation = useMutation({
    ...
    mutationFn: (data: object) =>
      users.update(privateFetch, userId, data),
    onSuccess: () => {
      // invalidate all user queries
      queryClient.invalidateQueries({ queryKey: ["users", userId.toString()] });
      queryClient.invalidateQueries({ queryKey: ["user-decks", userId.toString()] });
  ...
  const refreshUserStatsMutation = useMutation({
    ...
    mutationFn: () =>
      users.update(privateFetch, userId, {
        score: 0,
        level: 0,
        games_played: 0,
      }),
